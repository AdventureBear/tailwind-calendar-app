...
function Day({day, colIndex, rowIndex}) {
    ...
    const isCurrentDay = day.format("MM-DD-YY") === dayjs().format("MM-DD-YY");
    const isCurrentWeek = day.week() === dayjs().week();
    ...
    useEffect(() => {
        if (savedEvents) {
            const events = savedEvents.filter((evt) => {
                return dayjs(evt.day).format("MM-DD-YY") === day.format("MM-DD-YY")
            })
    ...
    //when labels changes, filter events to show only the checked categories
    useEffect(() => {
       // console.log("Labels: ", labels)
        if(dayEvents){
            const filteredEvents = dayEvents.filter((evt)=> {
                 const label = labels.find((lbl) => lbl.label === evt.label)
        ...
        `}>
                    ...
                    >
                        {filteredEvents.map((evt, i)=> {
                            const dayClassName= labelsClasses[evt.label] + ' p-1 mx-1 text-gray-600 text-sm rounded mb-1 truncate'
                            return (
                                <div
                                    onClick={() => {
                                        setSelectedEvent(evt); // Set the selected event here
                                    }}
                                    key={i}
            ...
            { calendarView ==="weekly" &&
                <>
                   <div className={` ${getColStartStyle(colIndex+1)} flex flex-col items-center border-gray-200 border-l-2 border-l-gray-200`}>
